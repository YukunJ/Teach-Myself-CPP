...
bool spmc_queue_enqueue(spmc_queue_t *queue, uint8_t *src_data) {
  ...
  int64_t reader_idx = atomic_load(&queue->shared->reader_idx);
  int64_t writer_idx = atomic_load(&queue->shared->writer_idx);
  ...
  atomic_fetch_add(&queue->shared->writer_idx, 1);
  return true;
}
...
bool spmc_queue_dequeue(spmc_queue_t *queue, uint8_t *dst_data) {
  ...
  atomic_fetch_add(&queue->shared->reader_idx, 1);
  return true;
}